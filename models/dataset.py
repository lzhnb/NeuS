...
class Dataset:
    def __init__(self, conf: omegaconf.OmegaConf):
        ...
        self.camera_outside_sphere = conf.get("camera_outside_sphere", True)
        self.scale_mat_scale = conf.get("scale_mat_scale", 1.1)
        ...
        self.world_mats_np = [
            camera_dict["world_mat_%d" % idx].astype(np.float32) for idx in range(self.n_images)
        ]
        ...
        self.scale_mats_np = [
            camera_dict["scale_mat_%d" % idx].astype(np.float32) for idx in range(self.n_images)
        ]
        ...
        object_scale_mat = np.load(os.path.join(self.data_dir, self.object_cameras_name))[
            "scale_mat_0"
        ]
        object_bbox_min = (
            np.linalg.inv(self.scale_mats_np[0]) @ object_scale_mat @ object_bbox_min[:, None]
        )
        object_bbox_max = (
            np.linalg.inv(self.scale_mats_np[0]) @ object_scale_mat @ object_bbox_max[:, None]
        )
