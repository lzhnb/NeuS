...
class Runner:
    def __init__(self, conf_path: str, mode="train", case="CASE_NAME", is_continue=False) -> None:
        ...
        self.warm_up_end = self.conf.train.get("warm_up_end", 0)
        self.anneal_end = self.conf.train.get("anneal_end", 0)
        # Weights
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            color_fine_loss = (
                F.l1_loss(color_error, torch.zeros_like(color_error), reduction="sum") / mask_sum
            )
    ...
    def validate_image(self, idx=-1, resolution_level=-1):
        ...
        if len(out_rgb_fine) > 0:
            img_fine = (np.concatenate(out_rgb_fine, axis=0).reshape([H, W, 3, -1]) * 256).clip(
                0, 255
            )
        ...
        if len(out_normal_fine) > 0:
            ...
            normal_img = (
                np.matmul(rot[None, :, :], normal_img[:, :, None]).reshape([H, W, 3, -1]) * 128
                + 128
            ).clip(0, 255)
    ...
    def interpolate_view(self, img_idx_0, img_idx_1):
        ...
        writer = cv.VideoWriter(
            os.path.join(
                video_dir, "{:0>8d}_{}_{}.mp4".format(self.iter_step, img_idx_0, img_idx_1)
            ),
            fourcc,
            30,
            (w, h),
